-- Update handle_new_user function to use email as welcome flow name
...
declare
  ...
  v_flow_id uuid;
begin
  -- Insert account with owner_id = new.id (auth.uid()) and NULL name (nullable)
  ...
  -- Create default welcome flow for new account - use email as name
  insert into public.welcome_flows (account_id, name, enabled, is_default, workspace_id)
  values (v_account_id, new.email, true, true, 'default')
